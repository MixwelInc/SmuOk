  ...
  public partial class SupplyDate : UserControl
  {
    ...
    public void SpecList_CheckedChanged(object sender, EventArgs e)
    {
      DB.SpecList_CheckedChanged(sender, FormIsUpdating);
        ...
        {
            MsgBox("Инструкция:\n 1.Для удаления записи введите ее идентификатор из поля 'ID з. на пост.', находящейся в выгрузке на этой вкладке; \n " +
                    "2. Для удаления записей, которые остались только на вкладках 'Заявка на поставку'/'Согласование счета' введите их идентификатор из столбца 'ID позиции заявки';");
            return;
        }
    }
}
