    ...
    public void FillFilling()
    {
      ...
      /*MyFillDgv(dgvSpecFill, q);
            ...
            for (int i = 0; i < dgvSpecFill.Rows.Count; i++)
            {
                ...
                if(checknum == 0)
                {
                    string ins = "\ninsert into SupplyOrder (SOId,SOFill) \nValues (" + icid + "," + val2 + ");";
                    MyExecute(ins);
                }
        ...
        " CASE WHEN sf.SFQtyBuy>0 THEN 'Подрядчик' ELSE 'Заказчик' END SOSupplierType," +
        //" SOId," +
        " so.SOResponsOS, SOOrderNum, SOOrderDate, so.SOWishDates as SFEOStartDate, so.SOQties as SFEOQty, SOPlan1CNum, SO1CPlanDate, SOComment" +
        " from" + 
        " SpecFill sf" +
        " left join SupplyOrder so on sf.SFId = SOFill" + //проверить, корректно ли тут грузится, потому что в бд все есть в таблице sfeo
        " left join vwSpecFill vw on sf.SFId = vw.SFId" +
        " left join Spec s on s.SId = vw.SId" +
        " left join SpecFillExec sfe on sf.SFId=SFEFill" +//
        " left join Executor e on e.eid = sfe.sfeexec" +
