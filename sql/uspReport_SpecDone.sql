...
ALTER PROCEDURE [dbo].[uspReport_SpecDone]     
 @spec nvarchar(max)    
 ...
 if @do_report=0     
  begin    
   ...
   select    
    ...
    null [6], null [7], null [8], null [9], case when sv.svid = max_sv.id then 'new' else 'old' end as [10], vwsf.SFRemovedPos [11]   
   from SpecVer sv    
  ...
  select    
   sfe.SFEId [-3],EName [-2],vwsf.SFSupplyPID [-1], [Чьи материалы] [0],SFNo+''.''+SFNo2 [1],SFName [2],SFMark [3],SFUnit [4],SFEQty [5], DSumQty [6], null [7], null [8], dd.capt [9],    
     case when sv.svid = max_sv.id then ''new'' else ''old'' end as [10], vwsf.SFRemovedPos [11] ,'+@PivotSelectColumnNames+'    
   from SpecVer sv    
   inner join vwSpecFill vwsf on sv.SVId=vwsf.SVId    
   ...
   outer apply( select top(1) d2.DCaption capt from Done d2 where d2.DSpecExecFill = d.DSpecExecFill order by d2.DId DESC)dd
